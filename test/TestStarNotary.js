...
it("can add the star name and star symbol properly", async () => {
  ...
  const name = "Star Notary Sundeep";
  const symbol = "SNS";
  const instance = await StarNotary.new(name, symbol);
  await instance.createStar("Pole star", 26);
...
it("lets 2 users exchange stars", async () => {
  ...
  const [tokenId1, tokenId2] = [26, 97];

  // 1. create 2 Stars with different tokenId
  await instance.createStar("Rigel", tokenId1, { from: owner1 });
  await instance.createStar("Sirius", tokenId2, { from: owner2 });
...
it("lets a user transfer a star", async () => {
  ...
  const [owner1, owner2] = accounts;
  const tokenId = 12;
  await instance.createStar("Procyon", tokenId, { from: owner1 });
...
it("lookUptokenIdToStarInfo test", async () => {
  const instance = await StarNotary.deployed();
  // 1. create a Star with different tokenId
  const { name, tokenId } = { name: "Achemar", tokenId: 96 };
